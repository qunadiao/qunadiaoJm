...
//
    ...
    */
    ...
    func searchBar(searchBar:UISearchBar,textDigChange searchText:String){
        print(searchText)
        // 没有搜索内容格式显示全部组件
        if searchText == ""{
            self.searchresults = self.fishingSpots
        }else {
            // 匹配用户输入内容的前缀（不区分大小写）
            self.searchresults = []
            for object in self.fishingSpots {
                if object.name.lowercaseString.hasPrefix(searchText.lowercaseString){
